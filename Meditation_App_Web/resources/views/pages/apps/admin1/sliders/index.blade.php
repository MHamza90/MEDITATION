<x-default-layout>
    ...
    @endsection
    @section('breadcrumbs')
        {{ Breadcrumbs::render('app-management.slider.index') }}
    @endsection
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                                ...
                                <form id="kt_modal_export_users_form" class="form"
                                    action="{{ route('app-management.slider.store') }}" method="POST"
                ...
                <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_users">
                    <thead>
                        <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
                            ...
                            @can('write slider management')
                            <th class="min-w-125px">Status</th>
                            @endcan
                            @canany(['write slider management', 'delete slider management'])
                        ...
                        @isset($data)
                            @foreach ($data as $item)
                                <tr>
                                    ...
                                    @endcan
                                    <td class="text-end">
                                        @can('delete slider management')
                                        <form action="{{ route('app-management.slider.destroy', $item->id) }}" method="post" style="display:inline">
                                            @csrf
                                            @method('DELETE')
                                        ...
                                        @endcan
                                        @can('write slider management')
                                        <form action="{{ route('app-management.slider.edit', $item->id) }}" method="get"
    ...
    <script type="text/javascript">
        ...
        $(".switch-input").change(function() {
            ...
            if (this.checked)
                ...
                var status = 0;
            $.ajax({
                url: "{{ route('app-management.slider.change.status') }}",
                type: 'GET',
                /*dataType: 'json',*/
                data: {
                    'id': this.id,
